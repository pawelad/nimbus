...
3. **Internal DNS (AdGuard + Headscale Split DNS):** To prevent latency bottlenecks when traveling, normal DNS queries use fast, local ISPs. However, Headscale's split DNS routes `home` and `pipusznicy.cloud` zones to Kif's AdGuard via the secure VPN. AdGuard returns dual A records (Tailscale IP first, LAN IP second) for each internal domain. Tailscale IP is preferred because remote devices (e.g., on a travel router at an Airbnb) could otherwise collide with devices on common subnets like `192.168.1.0/24`. LAN-only devices at home (e.g., smart TVs) fall back to the LAN IP after a brief timeout.
