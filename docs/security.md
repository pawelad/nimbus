...
- **Dynamic Home IP Whitelisting:** To prevent accidental bans or false positives from your home connection (e.g., from local home automation API requests or user browsing), the deployment pipeline dynamically resolves your home's external IP via `api.ipify.org`. It templates this IP into a parser whitelist (`home-whitelist.yaml`) and automatically deletes any existing bans for that IP on the CrowdSec engine during deployment (`cscli decisions delete`).
